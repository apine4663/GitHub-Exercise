...
function valTimesIndex(arr){
    const newArr = arr.map(function(num,idx){
        return num*idx;
    })
    return newArr;
}
...
function find(arr, searchValue) {
    let val =arr.filter(function(char){
        return searchValue===char
    });
    return val[0];
}
...
function doubleOddNumbers(arr) {
    return newArr=  arr.filter(function(odd){
        return odd%2!==0
    })
    . map(function(num){
        return num*2;
